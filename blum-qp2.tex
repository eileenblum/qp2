...
There are grammatical Finnish words, such as in (\ref{finnish.ruveta}a)
however, which contain two +back harmonizing vowels with a transparent
vowel between them. The NCC prevents Finnish from generating such words
by spreading a single +back feature across a -back feature and the OCP
allows multiple iterations of a +back feature to occur as long as each
is adjacent to a -back feature. So, the assimilation of the +back
features in words like (\ref{finnish.ruveta}a) must be due to something
other than spreading because the two +back features are not merged
during concatenation. I will call this other assimilation mechanism
\emph{agreement}. The analysis of Finnish agreement provided here
demonstrates that agreement creates local structures on the surface.
Transparent vowels are associated to the same features on each tier as
all other vowels and so identical +back features at any distance from
each other are connected via adjacency to the -back feature(s) between
them, as shown in (\ref{finnish.agree}).
...
mechanisms have been discovered to generate vowel harmony patterns and
