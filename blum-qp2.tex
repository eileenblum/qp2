...
The AR for the grammatical Turkish word {[}\textipa{köy\LARGE+}ün{]}
`village (Gen.sg)', shown in (\ref{turkish.koyun}a), contains a -high
root-final vowel and a +high suffix vowel. Both vowels are associated to
a single +round feature, which demonstrates full round harmony. The
hypothetical Turkish word, {[}köy-in{]} in (\ref{turkish.koyun}b),
contains the forbidden structure of (\ref{fsc.turkishround}) in bold and
red because it does not demonstrate full round harmony; the +high suffix
vowel is associated to a different round feature than the root-final
vowel.
...
generate an unattested sour grapes pattern over multi-tiered ARs.
...
the \enquote{blocking} vowel occurs somewhere within the word the
harmonizing feature (\(\pm\)F) will not be shared between all vowels on
the surface. In other words, no word will have full F harmony and a
\enquote{blocking} vowel. If all harmonizing vowels are also associated
to a single feature on a different tier (-G, for example) then a
\enquote{blocking} vowel would be one that is associated to a feature
with the opposite value on that tier (+G). The AR in (\ref{sgnofh})
shows two examples of full F harmony patterns that would not be found in
L\textsubscript{sg} and (\ref{sgexar}) shows an AR without full F
harmony that could be found in L\textsubscript{sg}. Ellipses in place of
a V represent the possibility for any number of vowels to occur in the
position, all associated to the same features.

\begin{exe}
\ex{Full F harmony with +G vowel not found in L$_s$$_g$}\label{sgnofh}
  \begin{tikzpicture}[baseline=(current bounding box.north)]
...
{ 
(a) & |(a)| +F &           &          &                    & & (b) & |(b)| -F &                      & \\
    & |(c)| V  & |(d)| ... & |(e)| V  & $\notin$ L$_s$$_g$ & &     & |(f)| V  & |(g)| ... & |(h)| V  & $\notin$ L$_s$$_g$ \\
    & |(i)| -G &           & |(j)| +G &                    & &     & |(k)| -G &           & |(l)| +G & \\
};
  \draw foreach \x in {c, d, e} {(a.south) -- (\x.north)};
  \draw foreach \x in {c, d} {(\x.south) -- (i.north)};
  \draw (e.south) -- (j.north);
  \draw foreach \x in {f, g, h} {(b.south) -- (\x.north)};
  \draw foreach \x in {f, g} {(\x.south) -- (k.north)};
  \draw (h.south) -- (l.north);
  \draw[black,->] (i) -- (j);
  \draw[black,->] (k) -- (l);
  \end{tikzpicture}
\end{exe}
...
{ 
|(a)| +F &           & |(b)| -F &           &          & \\
|(c)| V  & |(d)| ... & |(e)| V  & |(f)| ... & |(g)| V  & $\in$ L$_s$$_g$ \\
|(h)| -G &           &          &           & |(i)| +G & \\
};
  \draw foreach \x in {c, d} {(a.south) -- (\x.north)};
  \draw foreach \x in {c, d} {(\x.south) -- (h.north)};
  \draw foreach \x in {e, f, g} {(b.south) -- (\x.north)};
  \draw foreach \x in {c, d, e, f} {(\x.south) -- (h.north)};
...
Two FSCs can captures the sour grapes pattern exemplified by
(\ref{sgnofh}) and (\ref{sgexar}). Together the FSCs forbid two vowels
from sharing a single F feature if they also share a -G feature that
precedes a +G feature associated to a third vowel, which shares the
single F feature. The NCC crucially prevents any two non-adjacent vowels
from being associated to the same F feature if a vowel between them is
associated to a different F feature. While the vowels are not ordered on
the melody tier in (\ref{sg.fsc}) because consonants can occur between
them, the NCC prevents any vowels from intervening that are associated
to different F features.
