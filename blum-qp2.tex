...
examples can be found in section 3.1.
...
out (\ref{turkish.tatil}a), which contains the forbidden structure shown
in bold and red. While (\ref{turkish.tatil}b) also violates the
hypothetical FSC in (\ref{fsc.turkish.nomorph}a), including a morpheme
boundary on the back tier prevents (\ref{turkish.tatil}b) from violating
the FSC posited in (\ref{fsc.turkishback}a) for Turkish back harmony
because the two different back features are on the same side of the
morpheme boundary.
