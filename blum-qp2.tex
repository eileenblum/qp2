...
features throughout a word or the agreement between non-adjacent vowels.
This qualifying paper will show that both spreading and agreement each
generate vowel harmony patterns on the surface, but all such patterns
...
to an assimilatory process that transforms underspecified underlying ARs
into harmonizing surface ARs. On the surface, the result of a spreading
process over ARs is a structure in which a single feature is associated
to multiple elements on the segmental tier, as in (\ref{spread}a). In
this paper, the term \enquote{spreading} will refer to the resulting
multiple association of features rather than the process that derives
such structures. Similarly, \enquote{agreement} is used here to refer to
...
the concatenation operation, as shown in (\ref{spread}b) and
(\ref{agree}b), respectively. In (\ref{spread}), adjacent identical
features are merged to create multiple association. In (\ref{agree}),
multiple iterations of a +F and +G feature are possible when a primitive
with a -F and -G feature intervenes between two primitives with +F and
+G features because each primitive is only concatenated with the
primitive directly adjacent to it.
...
capture both spreading and agreement surface patterns and to show that
enriching the representation with morphological and word boundaries
increases its expressive power. Thus vowel harmony can be considered a
single set of patterns despite being derived by different assimilation
processes because all surface vowel harmony patterns are generated by a
