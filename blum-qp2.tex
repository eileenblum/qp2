...
that results from the spreading of vowel features throughout a word.
...
result from a uniform theory of markedness constraints. This paper
analyzes only the surface ARs of vowel harmony patterns. In the vowel
...
to multiple elements on the segmental tier. In this paper, the term
\enquote{spreading} will refer to the resulting multiple association of
features rather than the process that derives such structures.
Similarly, \enquote{agreement} is used here to refer to surface ARs with
non-adjacent identical features. On the surface, both spreading and
agreement are derived via the concatenation operation, such that only
adjacent features are merged to create multiple association and multiple
iterations of a feature are possible when a primitive with a different
element on the same feature tier intervenes between two primitives with
identical features. However, it will be shown that assimilation via both
spreading and agreement is generated by FSCs. Thus vowel harmony can be
considered one set of patterns despite being derived by different
assimilation mechanisms because all such patterns are generated by a
