...
not on feature tiers. Such an analysis would not capture the Turkish
harmony patterns, however, because disharmonic roots are grammatical.
For example, if the morpheme boundary is removed from the feature tiers,
then the FSCs in (\ref{fsc.turkishback}) would look like those in
...
A grammatical Turkish root like {[}tatil{]} `vacation', however, would
violate (\ref{fsc.turkish.nomorph}a). Removing the morpheme boundary
from the back feature tier in (\ref{fsc.turkish.nomorph}a) would rule
out (\ref{turkish.tatil}a), which contains the forbidden structure show
in bold and red. On the other hand, including a morpheme boundary on the
back tier of (\ref{fsc.turkish.nomorph}) prevents (\ref{turkish.tatil})
from violating the constraint because the two different back features
are on the same side of the morpheme boundary, as shown in
(\ref{turkish.tatil}b).
