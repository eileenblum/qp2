...
as in a transformation (or map) from underlying to surface form. Some
...
element on another tier. Many scholars have thus viewed vowel harmony as
mapping an input with a vowel feature associated to one vowel onto an
output where that same feature is associated to multiple vowels.
...
In other words, the L\textsubscript{SG} vowel harmony pattern requires
either one feature on both the ATR and low tiers or multiple features on
both tiers. In order to rule out a possible AR like (\ref{sgexar}c), a
FSC would have to forbid a substructure with more than one ATR feature,
but only a single low feature. Referring to the number of features
present in the AR of a word requires reference to word boundaries, much
like how morpheme boundaries were necessary to capture the
morpheme-specific harmony pattern of Turkish. Word boundaries will be
represented using the \textipa{\LARGE$\sharp$} symbol. As with Turkish,
the argument could be made that word boundaries are represented only on
the segmental tier, as shown in (\ref{sg.bound}).

\begin{exe}
\ex{Word boundaries only represented on the segmental tier}\label{sg.bound} \\
  \begin{tikzpicture}[baseline=(current bounding box.north)]
  \matrix [matrix of nodes, row sep={3.45em,between origins}, column sep={3.5em,between origins}, nodes={text height=1em, text depth=0.5em}]
{
                     & |(b)| -ATR        &         & |(c)| +ATR &         &         &            &\\
|(1)| \LARGE$\sharp$ & |(2)| \textipa{O} & |(3)| b & |(4)| e    & |(5)| j & |(6)| e & |(7)| a    & |(8)| \LARGE$\sharp$ \\
                     & |(f)| -low        &         &            &         &         & |(g)| +low &\\
};
  \draw (b.south) -- (2.north);
  \draw (2.south) -- (f.north);
  ...
  \ex{Ungrammatical $L_{SG}$ word also generated} \label{sg.1low} \\
    \begin{tikzpicture}[baseline=(current bounding box.north)]
  \matrix [matrix of nodes, row sep={3.45em,between origins}, column sep={3.5em,between origins}, nodes={text height=1em, text depth=0.5em}]
{
                     & |(b)| -ATR        &         & |(c)| +ATR &         &         &         &\\
|(1)| \LARGE$\sharp$ & |(2)| \textipa{O} & |(3)| b & |(4)| e    & |(5)| j & |(6)| e & |(7)| i & |(8)| \LARGE$\sharp$ \\
                     & |(f)| -low        &         &            &         &         &         &\\
};
  \draw (b.south) -- (2.north);
  \draw foreach \x in {4, 6, 7} {(c.south) -- (\x.north)};
  \draw foreach \x in {2, 4, 6, 7} {(\x.south) -- (f.north)};
  \draw[black,->] (b) -- (c);
  \draw[black,->] (1) -- (2);
  \draw[black,->] (2) -- (3);
  \draw[black,->] (3) -- (4);
...
\noindent While the AR in (\ref{sg.bound}) would be grammatical in
L\textsubscript{SG}, including word boundaries only on the segmental
tier also generates the ungrammatical structure in (\ref{sg.1low}). In
order to distinguish (\ref{sg.bound}) from (\ref{sg.1low}) an FSC must
mark (\ref{sg.1low})--- and not (\ref{sg.bound}) ---as ungrammatical.
The only difference between these two ARs is that (\ref{sg.1low})
contains a single feature on the low tier that is associated to the
final vowel, while the final vowel in (\ref{sg.bound}) is associated to
a different low feature than the other vowels. So, it is necessary to
make reference to the number of features within a word and to do so---
without counting features or introducing an existential quantifier
---requires representing a domain restriction on feature tiers. If a
feature is adjacent to a word boundary on both sides then it is the only
feature present on that tier within the word. The FSC in (\ref{sg.fsc})
utilizes a word boundary on the low tier to forbid an AR with only a
single low feature and more than one ATR feature, which rules out the
ungrammatical AR in (\ref{sg.forbid}) because it contains the forbidden
substructure in bold and red. \newpage
...
$\ast$               & |(a)| +ATR & |(b)| -ATR \\
                     & |(1)| V    & \\
|(e)| \LARGE$\sharp$ & |(c)| -low & |(d)| \LARGE$\sharp$\\
};
\draw (a.south) -- (1.north);
\draw (1.south) -- (c.north);
\draw[black,->] (a) -- (b);
...
\draw[black,->] (e) -- (c);
  ...
  \matrix [matrix of nodes, row sep={3.45em,between origins}, column sep={3.5em,between origins}, nodes={text height=1em, text depth=0.5em}]
{
|(a)| \LARGE$\sharp$ & |[red](b)| \textbf{-ATR}        &         & |[red](c)| \textbf{+ATR} &         &         &         & |(d)| \LARGE$\sharp$\\
|(1)| \LARGE$\sharp$ & |[red](2)| \textbf{\textipa{O}} & |(3)| b & |(4)| e    & |(5)| j & |(6)| e & |(7)| i & |(8)| \LARGE$\sharp$ \\
|[red](e)| \textbf{\LARGE$\sharp$} & |[red](f)| \textbf{-low}        &         &            &         &         &         & |[red](g)| \textbf{\LARGE$\sharp$}\\
};
...
\subsection{Ruling out an unattested sour grapes
pattern}\label{ruling-out-an-unattested-sour-grapes-pattern}
