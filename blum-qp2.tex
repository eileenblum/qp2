...
multi-tiered ARs. The investigation will provide a well-defined,
...
However, this qualifying paper will show that--- on the surface ---vowel
harmony patterns can be generated by either spreading or agreement and
that both processes are captured by a uniform theory of markedness
constraints. It will thus be argued that vowel harmony is local.
...
vowel harmony patterns using forbidden k-factor grammars. Sets of
multi-tiered ARs can then be compared to existing subregular grammars to
classify vowel harmony patterns with respect to the subregular
hierarchy.
...
later be compared to two-tiered ARs of tone. Three aspects of
...
where that same feature is associated to multiple vowels.
...
adjacency of concatenated primitives. While assimilation is possible
over any distance on the segmental tier, the adjacency of features to a
...
ARs of vowel harmony. A goal shared by all of generative phonology is to
distinguish attested patterns from logically possible, but unattested
ones. The remaining section will show that the expressive power of
multi-tiered ARs is significantly increased by enriching the
representation because a sour grapes vowel harmony pattern can be
described by FSCs over multi-tiered ARs only if word boundaries are
included in the set of representations allowed on feature tiers. The
problem remains of distinguishing attested patterns--- like Turkish
which requires domain boundaries on feature tiers ---from unattested
patterns, like sour grapes.
...
grammatical in Akan. However, +low vowels block Akan vowel harmony to
their left; so an Akan-like sour grapes word would have a +low vowel and
the ATR feature would not spread on the left of that +low vowel, as in
(\ref{sgak}b). The surface ARs in (\ref{akan.sg}) show the difference
between an Akan and a related sour grapes pattern.

\begin{exe}
\ex{ATR harmony}\label{sgak} 
\begin{xlist}
  \ex Akan: /\textipa{O}bejei/ $\mapsto$ [\textipa{O}b\textipa{E}j\textipa{E}\textipa{I}] `he came and did it'
  \ex sour grapes: /\textipa{O}bejea/ $\mapsto$ [\textipa{O}bejea]
\end{xlist}
\end{exe}
...
|(c)| -low        &           &                   &           &                   &                        &     & |(d)| -low        &           &            &           &         & |(e)| +low \\
  ...
  \draw (a.south) -- (1.north);
  \draw foreach \x in {2, 3, 4} {(a.south) -- (\x.north)};
  \draw (1.south) -- (c.north);
...
initial -low vowel onto every -low vowel to its right. However, in
...
substructure with more than one ATR feature, but no +low feature, as in
(\ref{fsc.1low}). Referring to the number of features present in the AR
of a word without counting features requires reference to word
...
different ATR features and in which the -low feature is associated to
the word-final vowel regardless of the number of vowels in the word. The
ordering relation on the ATR tier is excluded because the same
constraint holds whether -ATR precedes or succeeds +ATR. The ARs in
(\ref{sg.bound}) and (\ref{sg.1low}) illustrate the difference between a
grammatical and an ungrammatical L\textsubscript{SG} word.
...
(a) $\ast$ & |(a)| -ATR & |(b)| +ATR &                  & (b) $\ast$ & |(d)| -ATR & |(e)| +ATR &                & \\
           & |(1)| V    & |(2)| V    & |(3)| \LARGE$\#$ &            & |(4)| V    & |(5)| V    & |(6)| V$^n$ & |(7)| \LARGE$\#$\\
           & |(c)| -low &            &                  &            & |(f)| -low &            &                & \\
};
  \draw (a.south) -- (1.north);
  \draw (b.south) -- (2.north);
...
\ex{Grammatical $L_{SG}$ AR}\label{sg.bound} \\
  \begin{tikzpicture}[baseline=(current bounding box.north)]
  \matrix [matrix of nodes, row sep={3.45em,between origins}, column sep={3.5em,between origins}, nodes={text height=1em, text depth=0.5em}]
{
                     & |(b)| -ATR        &         & |(c)| +ATR &         &         &            &\\
|(1)| \LARGE$\#$ & |(2)| \textipa{O} & |(3)| b & |(4)| e    & |(5)| j & |(6)| e & |(7)| a    & |(8)| \LARGE$\#$ \\
                     & |(f)| -low        &         &            &         &         & |(g)| +low &\\
};
  \draw (b.south) -- (2.north);
  \draw (2.south) -- (f.north);
  \draw foreach \x in {4, 6, 7} {(c.south) -- (\x.north)};
  \draw foreach \x in {2, 4, 6} {(\x.south) -- (f.north)};
  \draw (7.south) -- (g.north);
  \draw[black,->] (b) -- (c);
  \draw[black,->] (1) -- (2);
  \draw[black,->] (2) -- (3);
  \draw[black,->] (3) -- (4);
...
\ex{Ungrammatical $L_{SG}$ AR} \label{sg.1low} \\
  \begin{tikzpicture}[baseline=(current bounding box.north)]
  \matrix [matrix of nodes, row sep={3.45em,between origins}, column sep={3.5em,between origins}, nodes={text height=1em, text depth=0.5em}]
{
                     & |[red](b)| \textbf{-ATR}        &         & |[red](c)| \textbf{+ATR} &         &         &         &\\
|(1)| \LARGE$\#$ & |[red](2)| \textbf{\textipa{O}} & |(3)| b & |[red](4)| \textbf{e}    & |(5)| j & |[red](6)| \textbf{e} & |[red](7)| \textbf{i} & |[red](8)| \textbf{\LARGE$\#$} \\
                     & |[red](f)| \textbf-low        &         &            &         &         &         &\\
};
  \draw[thick,red] (b.south) -- (2.north);
  \draw[thick,red] (c.south) -- (4.north);
  \draw foreach \x in {6, 7} {(c.south) -- (\x.north)};
  \draw[thick,red] foreach \x in {2, 4, 6, 7} {(\x.south) -- (f.north)};
  \draw[black,->] (b) -- (c);
  \draw[black,->] (1) -- (2);
  \draw[black,->] (2) -- (3);
  \draw[black,->] (3) -- (4);
...
contains a +low feature that is associated to the final vowel, while
(\ref{sg.1low}) contains only a single -low feature that is also
associated to the final vowel.

Unlike in Lamont (2018), a sour grapes vowel harmony pattern can be
captured using FSCs over multi-tiered ARs with word boundaries only on
the segmental tier. Lamont (2018) found that two-tiered ARs were not
expressive enough to capture a sour grapes nasal harmony pattern and
posited that sour grapes must be generated by a Regular grammar over
two-tiered ARs. However, the interaction between multiple feature tiers
allows the ARs used here to express the so-called long distance blocking
effect of +low in L\textsubscript{SG}, as shown in (\ref{sg.bound}).
Even with word boundaries only on the segmental tier, sour grapes in
vowel harmony suggests that multi-tiered ARs might increase the
expressive power of a grammar as compared to two-tiered ARs. Future work
will directly compare the expressivity of multi-tiered ARs of vowel
harmony to two-tiered ARs of tone and nasal harmony.

\subsubsection{Boundaries on feature
tiers}\label{boundaries-on-feature-tiers}

While sour grapes can be captured by FSCs over multi-tiered ARs when
word boundaries are restricted to the segmental tier, section 4.1
demonstrated, for Turkish, that morpheme boundaries must be represented
on feature tiers as well. Extending this requirement to word boundaries
in a sour grapes pattern would simplify the FSCs needed to capture the
vowel harmony pattern in L\textsubscript{SG}. As mentioned above,
...
the FSCs in (\ref{fsc.1low}) above forbid a structure with two different
ATR features when a -low feature is associated to a vowel that is
adjacent to the final word boundary. However, with boundaries allowed on
feature tiers the FSC does not have to reference more than one vowel on
the segmental tier; rather, the -low feature itself can be designated as
word-final. As shown in (\ref{sg.fsc}), the sour grapes vowel harmony
pattern can thus be captured by forbidding a structure with two
different adjacent ATR features and a -low feature that is adjacent to a
final word boundary.
...
(a) $\ast$  & |(a)| +ATR & |(b)| -ATR       & (b) $\ast$ & |(e)| -ATR & |(f)| +ATR\\
  ...
  \ex{Ungrammatical $L_{SG}$ word also generated} \label{sg.forbid} \\
    \begin{tikzpicture}[baseline=(current bounding box.north)]
  \matrix [matrix of nodes, row sep={3.45em,between origins}, column sep={3.5em,between origins}, nodes={text height=1em, text depth=0.5em}]
{
|(a)| \LARGE$\#$ & |[red](b)| \textbf{-ATR}        &         & |[red](c)| \textbf{+ATR} &         &         &         & |(d)| \LARGE$\#$\\
|(1)| \LARGE$\#$ & |[red](2)| \textbf{\textipa{O}} & |(3)| b & |(4)| e    & |(5)| j & |(6)| e & |(7)| i & |(8)| \LARGE$\#$ \\
|[red](e)| \textbf{\LARGE$\#$} & |[red](f)| \textbf{-low}        &         &            &         &         &         & |[red](g)| \textbf{\LARGE$\#$}\\
};
  \draw[thick,red] (b.south) -- (2.north);
...
ASL\textsuperscript{g\textsubscript{T}} class. This section has shown
that including word boundaries on feature tiers allows an
L\textsubscript{SG} grammar to express a minimal restriction on the
number of features--- either one or more than one ---without counting or
existential quantification, which ensures the grammar remains
subregular.
