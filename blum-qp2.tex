...
different F features and a +G vowel, shown in (\ref{sgexar}b).
L\textsubscript{SG} also distinguishes (\ref{sgexar}a) and
(\ref{sgexar}b) from (\ref{sgexar}c), which contains different F
features, but no +G vowel.
...
In (\ref{sgexar}), the superscript `n' represents the possible number of
vowels that can occur in a given position with the same featural
associations. So a word like (\ref{sgexar}a) can have any number of
vowels that are associated to both -F and -G before a +G vowel. However,
in order to capture the (\ref{sgexar}a) pattern, a grammar must be able
to search over any distance to find a +G vowel and determine the F
features for vowels an arbitrary distance away. A negative constraint
that could rule out (\ref{sgexar}c) would have to either count the
number of vowels that precede the +F vowel to determine the distance at
which a vowel can no longer be associated to -G or an existential
quantifier is required to say that some +F vowel exists, which cannot be
associated to -G.
