...
spreading, copied below in (\ref{nsg}).
...
multi-tiered ARs of vowel harmony. A goal shared by all of generative
phonology is to distinguish attested patterns from logically possible,
but unattested ones. The remaining section will show that a sour grapes
vowel harmony pattern can be described by FSCs over multi-tiered ARs
only if word boundaries are included in the set of representations
...
the ATR feature would not spread to any vowel on the left of that +low
vowel, as in (\ref{sgak}b). The surface ARs in (\ref{akan.sg}) show the
...
The ARs in (\ref{sg.bound}) and (\ref{sg.1low}) illustrate the
difference between a grammatical and an ungrammatical
L\textsubscript{SG} word.
...
(\ref{sg.1low}) contains only a single -low feature associated to the
final vowel.
...
in a sour grapes pattern simplifies the FSCs needed to capture the vowel
harmony pattern in L\textsubscript{SG} by decreasing the number of
vowels represented on the segmental tier. As mentioned above,
...
feature tiers the FSC in (\ref{sg.fsc}) does not need to reference more
than one vowel on the segmental tier; rather, the -low feature itself is
designated as word-final. As shown in (\ref{sg.fsc}), the sour grapes
vowel harmony pattern can thus be captured by forbidding a structure
with two different ATR features and a -low feature that is adjacent to a
final word boundary. The ordering relation is excluded on the ATR tier
because the same constraint holds regardless of the order in which the
two ATR features occur.
...
Unlike with boundaries only on the segmental tier, when word boundaries
are represented on feature tiers L\textsubscript{SG} is captured by a
single simpler FSC; however, L\textsubscript{SG} can be captured with
FSCs over multi-tiered ARs using both representations. The FSC in
(\ref{sg.fsc}) includes two different ATR features, but it differs from
the FSCs in (\ref{fsc.1low}) because it includes only two associations
to a single vowel and the -low feature is adjacent to a word boundary.
However, the FSC in (\ref{sg.fsc}) is able to capture the same
constraint against a word-final -low feature. The AR of the hypothetical
L\textsubscript{SG} word {[}\textipa{O}bejei{]} in (\ref{sg.forbid}),
for example, is marked ungrammatical because it contains the forbidden
structure of (\ref{sg.fsc}) with a -low feature adjacent to a final word
boundary in bold and red. This same word in (\ref{sg.1low}) is marked
ungrammatical by the FSC in (\ref{fsc.1low}b). So the sour grapes vowel
harmony pattern of L\textsubscript{SG} can be captured using FSCs over
multi-tiered ARs whether word boundaries are represented on the
segmental tier only or on both the segmental and feature tiers.
