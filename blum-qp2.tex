...
vowel associated to the same feature with a different explain that a
domain boundary primitive must also include that boundary on feature
tiers.
...
vowels, like {[}\textipa{k1z\LARGE+}\textipa{lar1n}{]} `girls (gen.)',
both suffix vowels are associated to the same back feature on the
surface, as shown in the AR (\ref{turkish.dishsuf}a).
...
However, Turkish also has grammatical words with disharmonic roots and
the FSCs in (\ref{fsc.turkish.nomorph}) do not discriminate between root
and suffix vowel features. A grammatical Turkish root like {[}tatil{]}
`vacation' would violate (\ref{fsc.turkish.nomorph}a) because it
contains the forbidden structure shown in bold and red in
(\ref{turkish.tatil}a).
...
\noindent The AR in (\ref{turkish.tatil}b) also violates
(\ref{fsc.turkish.nomorph}a), which thus forbids a grammatical Turkish
root. So the FSCs in (\ref{fsc.turkish.nomorph}) are able to capture the
Turkish suffix harmony pattern without representing morpheme boundaries
on a feature tier, but they are not able to capture disharmonic roots in
Turkish. Including the morpheme boundary on the feature tier
distinguishes the two back features as root features, such that
(\ref{turkish.tatil}b) does not violate the FSCs posited for Turkish in
(\ref{fsc.turkishback}).
