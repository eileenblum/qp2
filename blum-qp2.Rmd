...
A straightforward pattern of backness harmony, like in Turkish, can also be written as an FSC that forbids two vowels from being associated to different back features on either side of a morpheme boundary, as in (\ref{fsc.turkishback}). The melody tier does not include a successor ordering relation because the vowels can be separated from one another and from the morpheme boundary by one or more consonants. In the FSC in (\ref{fsc.turkishback}a), the +back vowel and the -back vowel must also be identifiable as the root and suffix vowels, respectively. The successor ordering relation on the back tier ensures that the +back vowel is to the left and the -back vowel is to the right. In addition, the morpheme boundary must be represented on the back tier in order to distinguish the root feature, and the vowels by association, from the suffix feature. The same reasoning holds for the FSC in (\ref{fsc.turkishback}b), but the root-final vowel is associated to a -back and the suffix vowel is associated to a +back feature.
...
\ex{\textipa{ip\LARGE+}ler `rope (Nom.pl)'}\label{turkish.ipler} \\
  ...
  \matrix [matrix of nodes, row sep={3.45em,between origins}, column sep={2.25em,between origins}, nodes={text height=1em, text depth=0.5em}] 
  {
(a) & |(a)| -back &              &               &         &         &         & (b) * & |[red](h)| \textbf{-back} &         & |[red](1)| \textbf{\LARGE+} &         & |[red](i)| \textbf{+back} & \\ 
    & |(b)| i     & |(c)| p      & |(d)| \LARGE+ & |(e)| l & |(f)| e & |(g)| r &       & |[red](j)| \textbf{i}     & |(k)| p & |(l)| \LARGE+               & |(m)| l & |[red](n)| \textbf{a} & |(o)| r \\
  };
...
\noindent The Turkish word [\textipa{ip\LARGE+}ler] `rope (Nom.pl)' illustrates full back harmony, as shown in (\ref{turkish.ipler}a). The hypothetical word [\textipa{ip\LARGE+}lar], on the other hand, is ungrammatical because the root and suffix vowels are associated to different back features and so the AR in (\ref{turkish.ipler}b) contains the forbidden substructure of (\ref{fsc.turkishback}b) in bold and red.
...
(a) &         & |(a)| -high  &         & |(1)| \LARGE+ & |(g)| +high &         & (b) *   &          & |(h)| -high                &         & |(3)| \LARGE+ & |[red](i)| \textbf{+high} & \\
    & |(b)| k & |(c)| ö      & |(d)| y & |(s)| \LARGE+ & |(e)| ü      & |(f)| n &         & |(j)| k & |[red](k)| \textbf{ö}      & |(l)| y & |(m)| \LARGE+              & |[red](n)| \textbf{i} & |(o)| n \\
    &         & |(p)| +round &         &                &             &         &         &        & |[red](q)| \textbf{+round} &         & |[red](2)| \textbf{\LARGE+} & |[red](r)| \textbf{-round} \\         
  };
  \draw (a.south) -- (c.north);
