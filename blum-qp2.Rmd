...
\end{exe}
 ...
 The surface requirement that +round and +low vowels share the same back feature can also be stated negatively as a constraint that forbids either a +round or a +low vowel from being associated to a different preceeding back feature. Together, the two FSCs in (\ref{fsc.finnish}) generate this negative constraint and the Finnish vowel harmony pattern. The ordering relation on the segmental tier of the FSCs is omitted because the vowels can have consonants between them, as in (\ref{finnish.pouta}). The ordering relation on the back tier, however, is crucial in order to allow transparency of certain -back vowels.
...
\end{exe} 
  ...
  On the surface, all Turkish suffix vowels are associated to the same back feature as root-final vowels and the FSCs in (\ref{fsc.turkish.nomorph}) forbid any two adjacent back features from having different values. In a grammatical Turkish word with two suffix vowels, like [\textipa{k1z\LARGE+}\textipa{lar1n}] `girls (gen.)', both suffix vowels are associated to the same back feature on the surface, as shown in the AR (\ref{turkish.dishsuf}a).
...
(b) & *        & |(d)| +back        &          &                &          & |[red](f)| \textbf{+back} &          &|[red](g)| \textbf{-back} & \\
...
(c) & *        & |(h)| +back        &          &                &          & |[red](j)| \textbf{-back} &          &|[red](k)| \textbf{+back} & \\
  ...
  \draw foreach \x in {6, 8} {(c.south) -- (\x.north)};
  \draw (d.south) -- (11.north);
  \draw (f.south) -- (15.north);
...
\noindent The ARs in (\ref{turkish.dishsuf}b) and (\ref{turkish.dishsuf}c), on the other hand, violate the FSCs in (\ref{fsc.turkish.nomorph}a) and (\ref{fsc.turkish.nomorph}b), respectively. In (\ref{turkish.dishsuf}b) the suffix vowel closest to the morpheme boundary is associated to a +back feature that is adjacent to a -back feature, shown in bold and red. Similarly, in (\ref{turkish.dishsuf}c) the suffix vowel closest to the morpheme boundary is associated to a -back vowel that is adjacent to a +back vowel, shown in bold and red. Thus the FSCs in (\ref{fsc.turkish.nomorph}), without morpheme boundaries on feature tiers, do capture the suffix harmony pattern in Turkish.
  ...
  However, Turkish also has grammatical words with disharmonic roots and the FSCs in (\ref{fsc.turkish.nomorph}) do not discriminate between root and suffix vowel features. A grammatical Turkish root like [tatil] `vacation' would violate (\ref{fsc.turkish.nomorph}a) because it contains the forbidden structure shown in bold and red in (\ref{turkish.tatil}a). <!-- Previous analysis captures Turkish with morpheme boundary on feature tiers
...
\noindent The AR in (\ref{turkish.tatil}b) also violates (\ref{fsc.turkish.nomorph}a), which thus forbids a grammatical Turkish root. So the FSCs in (\ref{fsc.turkish.nomorph}) are able to capture the Turkish suffix harmony pattern without representing morpheme boundaries on a feature tier, but they are not able to capture disharmonic roots in Turkish. Including the morpheme boundary on the feature tier distinguishes the two back features as root features, such that (\ref{turkish.tatil}b) does not violate the FSCs posited for Turkish in (\ref{fsc.turkishback}). 
