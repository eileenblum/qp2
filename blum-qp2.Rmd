...
## Assimilation Mechanisms
  ...
  Vowel harmony has previously been analyzed as an assimilatory process that results from the spreading of vowel features throughout a word. This qualifying paper will show that both spreading and agreement generate vowel harmony patterns on the surface, but all such patterns result from a uniform theory of markedness constraints. This paper analyzes only the surface ARs of vowel harmony patterns. In the vowel harmony literature, the term 'spreading' has generally referred to an assimilatory process that transforms underspecified underlying ARs into harmonizing surface ARs. On the surface, the result of a spreading process over ARs is a structure in which a single feature is associated to multiple elements on the segmental tier. In this paper, the term 'spreading' will refer to the resulting multiple association of features rather than the process that derives such structures. Similarly, 'agreement' is used here to refer to surface ARs with non-adjacent identical features. On the surface, both spreading and agreement are derived via the concatenation operation, such that only adjacent features are merged to create multiple association and multiple iterations of a feature are possible when a primitive with a different element on the same feature tier intervenes between two primitives with identical features. However, it will be shown that assimilation via both spreading and agreement is generated by FSCs. Thus vowel harmony can be considered one set of patterns despite being derived by different assimilation mechanisms because all such patterns are generated by a single theory of markedness.
