...
Another phenomenon commonly discussed in autosegmental spreading literature is an unattested, but logically possible, pattern called sour grapes in which the presence of a single blocking segment prevents any spreading within the word [@mccarthy2011; @lamont2018]. Following @lamont2018, this section will evaluate the expressive power needed to generate an unattested sour grapes pattern over multi-tiered ARs.

  In principle, a word in a hypothetical language with sour grapes (L~sg~) could have any number of syllables, but as long as the "blocking" vowel occurs somewhere within the word the harmonizing feature ($\pm$F) will not be shared between all vowels on the surface. In other words, no word will have full F harmony and a "blocking" vowel. If all harmonizing vowels are also associated to a single feature on a different tier (-G, for example) then a "blocking" vowel would be one that is associated to a feature with the opposite value on that tier (+G). The AR in (\ref{sgnofh}) shows two examples of full F harmony patterns that would not be found in L~sg~ and (\ref{sgexar}) shows an AR without full F harmony that could be found in L~sg~. Ellipses in place of a V represent the possibility for any number of vowels to occur in the position, all associated to the same features.
...
\end{exe}
  ...
  Two FSCs can captures the sour grapes pattern exemplified by (\ref{sgnofh}) and (\ref{sgexar}). Together the FSCs forbid two vowels from sharing a single F feature if they also share a -G feature that precedes a +G feature associated to a third vowel, which shares the single F feature. The NCC crucially prevents any two non-adjacent vowels from being associated to the same F feature if a vowel between them is associated to a different F feature. While the vowels are not ordered on the melody tier in (\ref{sg.fsc}) because consonants can occur between them, the NCC prevents any vowels from intervening that are associated to different F features.
