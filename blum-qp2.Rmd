...
A grammatical Turkish root like [tatil] `vacation', however, would violate (\ref{fsc.turkish.nomorph}a). Removing the morpheme boundary from the back feature tier in (\ref{fsc.turkish.nomorph}a) would rule out (\ref{turkish.tatil}a), which contains the forbidden structure show in bold and red. On the other hand, including a morpheme boundary on the back tier of (\ref{fsc.turkish.nomorph}) prevents (\ref{turkish.tatil}) from violating the constraint because the two different back features are on the same side of the morpheme boundary, as shown in (\ref{turkish.tatil}b). <!-- Previous analysis captures Turkish with morpheme boundary on feature tiers
...
Turkish vowel harmony can also be considered local because both its back and round harmony patterns are captured by the connected FSCs in (\ref{fsc.turkishback}) and (\ref{fsc.turkishround}). These FSCs refer to the associations between vowels and features as well as the ordering relation between features and a morpheme boundary.
