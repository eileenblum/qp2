...
## Assimilation Mechanisms
  ...
  Vowel harmony has previously been analyzed as an assimilatory process that results in multiple vowels being associated to the same feature on the surface. Such assimilation can result from the spreading of vowel features throughout a word or the agreement between non-adjacent vowels. This qualifying paper will show that both spreading and agreement each generate vowel harmony patterns on the surface, but all such patterns are captured by a uniform theory of markedness constraints. In the vowel harmony literature, the term 'spreading' has generally referred to an assimilatory process that transforms underspecified underlying ARs into harmonizing surface ARs. On the surface, the result of a spreading process over ARs is a structure in which a single feature is associated to multiple elements on the segmental tier, as in (\ref{spread}a). In this paper, the term 'spreading' will refer to the resulting multiple association of features rather than the process that derives such structures. Similarly, 'agreement' is used here to refer to surface ARs with non-adjacent identical features, as in (\ref{agree}a). 
  
\begin{exe}
\ex Spreading \label{spread}\\
  \begin{tikzpicture}[baseline=(current bounding box.north)]
  \matrix [matrix of nodes, row sep=2.5ex, column sep=2.25ex, nodes={text height=1em, text depth=0.5em}]
  {
(a) & |(a)| +F &         &         & & (b) & |(A)| +F &                                     & |(B)| +F &                                     & |(C)| +F \\
    & |(1)| V  & |(2)| V & |(3)| V & &     & |(4)| V  & \node {}; \draw(0, 0) circle (3pt); & |(5)| V  & \node {}; \draw(0, 0) circle (3pt); & |(6)| V \\
    & |(b)| +G &         &         & &     & |(D)| +G &                                     & |(E)| +G &                                     & |(F)| +G \\
  };
  \draw foreach \x in {1, 2, 3} {(a.south) -- (\x.north)};
  \draw foreach \x in {1, 2, 3} {(\x.south) -- (b.north)};
  \draw (A.south) -- (4.north);
  \draw (B.south) -- (5.north);
  \draw (C.south) -- (6.north);
  \draw (4.south) -- (D.north);
  \draw (5.south) -- (E.north);
...
\ex Agreement \label{agree}\\
  ...
  \matrix [matrix of nodes, row sep=2.5ex, column sep=2.25ex, nodes={text height=1em, text depth=0.5em}]
  {
(a) & |(a)| +F & |(b)| -F & |(c)| +F & & (b) & |(A)| +F &                                     & |(B)| -F &                                     & |(C)| +F \\
    & |(1)| V  & |(2)| V  & |(3)| V  & &     & |(4)| V  & \node {}; \draw(0, 0) circle (3pt); & |(5)| V  & \node {}; \draw(0, 0) circle (3pt); & |(6)| V \\
    & |(d)| +G & |(e)| -G & |(f)| +G & &     & |(D)| +G &                                     & |(E)| -G &                                     & |(F)| +F \\
  };
  \draw (a.south) -- (1.north);
  \draw (b.south) -- (2.north);
  \draw (c.south) -- (3.north);
  \draw (1.south) -- (d.north);
  \draw (2.south) -- (e.north);
  \draw (3.south) -- (f.north);
  \draw (A.south) -- (4.north);
  \draw (B.south) -- (5.north);
  \draw (C.south) -- (6.north);
  \draw (4.south) -- (D.north);
...
\noindent On the surface, both spreading and agreement are derived via the concatenation operation, as shown in (\ref{spread}b) and (\ref{agree}b), respectively. In (\ref{spread}), adjacent identical features are merged to create multiple association. In (\ref{agree}), multiple iterations of a +F and +G feature are possible when a primitive with a -F and -G feature intervenes between two primitives with +F and +G features because each primitive is only concatenated with the primitive directly adjacent to it. 

  It will be shown that vowel feature assimilation patterns that result from both spreading and agreement are local because they are generated by FSCs. FSCs are markedness constraints that represent the phonotactic restrictions of a language and can further demonstrate the expressive power of a particular representation. This paper will use FSCs to capture both spreading and agreement surface patterns and to show that enriching the representation with morphological and word boundaries increases its expressive power. Thus vowel harmony can be considered a single set of patterns despite being derived by different assimilation processes because all surface vowel harmony patterns are generated by a single theory of markedness.
