...
The analysis presented above captures both the Turkish back and round harmony patterns with FSCs in which morpheme boundaries are represented on all feature tiers. One critique of such an analsysis could be that morpheme boundaries can only be represented on the segmental tier and not on feature tiers. Such an analysis would not capture the Turkish harmony patterns, however, because disharmonic roots are grammatical. For example, if the morpheme boundary is removed from the feature tiers, then the FSCs in (\ref{fsc.turkishback}) would look like those in (\ref{fsc.turkish.nomorph}).
...
A grammatical Turkish root like [tatil] `vacation', however, would violate (\ref{fsc.turkish.nomorph}a). Removing the morpheme boundary from the back feature tier in (\ref{fsc.turkish.nomorph}a) would rule out (\ref{turkish.tatil}a), which contains the forbidden structure shown in bold and red. While (\ref{turkish.tatil}b) also violates the hypothetical FSC in (\ref{fsc.turkish.nomorph}a), including a morpheme boundary on the back tier prevents (\ref{turkish.tatil}b) from violating the FSC posited in (\ref{fsc.turkishback}a) for Turkish back harmony because the two different back features are on the same side of the morpheme boundary. <!-- Previous analysis captures Turkish with morpheme boundary on feature tiers
...
\ex{[tatil] `vacation'} \label{turkish.tatil} \\
  ...
  \matrix [matrix of nodes, row sep={3.45em,between origins}, column sep={3.25em,between origins}, nodes={text height=1em, text depth=0.5em}] 
  {
(a)     & |[red](1)| \textbf{+back} &         & |[red](2)| \textbf{-back} &         & (b)     & |(3)|  +back &         & |(4)| -back &         |(5)| \LARGE+ \\ 
|(a)| t & |(b)| a                   & |(c)| t & |(d)| i                   & |(e)| l & |(f)| t & |(g)| a      & |(h)| t & |(i)| i     & |(j)| l & |(k)| \LARGE+ \\
  };
