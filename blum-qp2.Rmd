...
\end{exe}
  ...
  A grammar that generates L~SG~ would distinguish (\ref{sgexar}a) and (\ref{sgexar}b) from (\ref{sgexar}c), which contains different F features, but no +G vowel. So a word like (\ref{sgexar}a) can have any number of vowels that are associated to both -F and -G before a +G vowel. An ASL^g^ grammar, however, does not distinguish (\ref{sgexar}a) and (\ref{sgexar}b) from (\ref{sgexar}c). 
